"""
Hybrid planner for the Agentico agent loop.
...
def _compose_reply(ticket: dict, context: dict) -> str:
    ...
    if customer.get("error") == "CUSTOMER_NOT_FOUND" and not order:
        return (
            f"Hi {first_name}, I could not match this request to an existing customer record yet. "
            "Please reply with your order number and the email address used for the order so we can help right away."
        )
...
def _deterministic_plan(ticket: dict, context: dict, tool_history: list[dict]) -> dict:
    ...
    if customer.get("error") == "CUSTOMER_NOT_FOUND" and not order:
        return {
            ...
            "reasoning": "The customer record could not be matched, so request clarification before taking order actions.",
        }
    ...
    if _is_policy_question(ticket) or (not order and "order" not in ticket.get("body", "").lower()):
        return {
            "action": "send_reply",
            "args": {"ticket_id": ticket["ticket_id"], "message": _compose_reply(ticket, context)},
            "reasoning": "This request can be answered directly from policy and available customer context.",
        }
...
def _build_llm_prompt(ticket: dict, context: dict, tool_history: list[dict], fallback: dict) -> str:
    ...
    return f"""
You are planning the NEXT SINGLE action for Agentico's support agent.
