"""
ReAct-style orchestration for Agentico support tickets.
"""

from __future__ import annotations
from core.sheets_sync import log_ticket_to_sheet
...
def _infer_order_id(ticket: dict, customer: dict) -> str | None:
    provided_order_id = ticket.get("order_id")
...
async def process_single_ticket_lifecycle(ticket: dict) -> None:
    ...
    try:
        ...
        if "error" not in str(customer_result):
            logger.set_refs(ticket_id, customer_ref=ticket.get("customer_id") or customer_result.get("customer_id"))
        ...
        if inferred_order_id:
            ...
            if "error" not in str(order_result):
                ...
                product_id = ticket.get("product_id") or order_result.get("product_id")
                if product_id:
        ...
        await finalize_ticket_and_flush_logs(
            ticket_id, executor, decision, logger, state_mgr,
...
async def finalize_ticket_and_flush_logs(
    ticket_id: str,
